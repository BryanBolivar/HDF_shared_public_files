...
function initialize_app( )
{
    ...
    $(".calendar-link").click(function(event) {
	    ...
	    add_event_to_calendar($("#event_id").val());
	    return false;
	});
...
function change_city( new_city )
{
	...
	device_get_json( "http://harddatafactory.com/techcal/" + current_city + "/event_data.js" );
    }
}
...
function loaddetail( eventid ) {
    ...
    $("#detail-Address").html( hdf_strings[ e[2] ] + "<br>" + hdf_strings[ e[3] ] );
    $("#detail-Desc").html( hdf_strings[ e[4] ] );
   
    if( e[7] == undefined ) {
	$("#detail-Image").css( "display", 'none');
    } else {
	$("#detail-Image").load(function(){scroll_reset_height( );});
	$("#detail-Image").attr( "src", "http://harddatafactory.com/techcal/" + current_city + "/thumbs/i" + e[7] + ".jpg" );
	$("#detail-Image").css( "display", 'inline');
    }

    $("#detail-URL").unbind("click");
    $("#detail-Source").unbind( "click" );

    $("#detail-Source").click(function(event) {
	    event.preventDefault();
	    device_open_url(hdf_strings[ e[6] ]);
	    return false;
	});

    $("#detail-URL").click(function(event) {
	    event.preventDefault();
	    device_open_url(hdf_strings[ e[5] ]);
	    return false;
	});
    ...
    $("#email-link").attr('to', '');
    $("#email-link").attr('subject', escape(emailTitle + " on " + emailDate));
    $("#email-link").attr('body', escape("Event: " + emailTitle + "\n" + "Date: " + emailDate + "\n" + "Location: " +  location + "\n" + "URL: " + linkURL + "\n" + "Source: " + srcURL + "\n" + description ));
    setup_email_phone_clicks( );

    //----------------------------------------------------------------------
    $("#monthcal").css("display", "none");
    $("#mytodaylist").css("display", "none");
    $("#mytoplist").css("display", "none");
...
function hidedetail( ) {
    $("#mytodaylist").css("display", "table");
    $("#mytoplist").css("display", "table");
...
function make_grid_cal( )
{
	...
	if( month_cnt > 0 ) {   // No left arrow if this is 1st month
	    html_str += "<a href='javascript:void(0)' onclick='set_date_to_first_day_of_this_month(" + (month_cnt-1) + ");' style='padding: 0 10px;'><img border=0 src='/public/sharedhtml/images/arrow_left.png'></a>";
	}
	...
	if( month_cnt < 2 ) { // No right arrow if this is last month
	    html_str += "<a href='javascript:void(0)' onclick='set_date_to_first_day_of_this_month(" + (month_cnt+1) + ");' style='padding: 0 10px;'><img border=0 src='/public/sharedhtml/images/arrow_right.png'></a>";
	}
...
function add_event_to_calendar(event_id)
{
    ...
    var start_date = get_date_from_text(start_date_str);
    var end_date_str = hdf_strings[event[9]];
    var end_date = get_date_from_text(end_date_str);
    var data = {
    ...
    if (event[8] == event[9]) {
	var time = hdf_strings[event[10]];
	var time_parts = time.split(":");
	var am_pm = event[11];
	var hours = parseInt(time_parts[0]);

	hours = hours %12 + (am_pm === "AM"? 0 : 12);
    ...
    device_add_event_to_calendar( data );
}
...
function setup_email_phone_clicks( )
{
    $(".email-link").unbind("click");
    $(".email-link").click( function( ) {    // In case there are several
	    device_send_email($(this).attr('to'),
			      $(this).attr('subject'),
			      $(this).attr('body') );
	});
    ...
    $(".phone-link").click( function( ) {    // In case there are several
	    device_call_phone_number( $(this).text( ) );
	});
}
